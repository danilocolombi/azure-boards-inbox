...
src/
  ...
  commands/
    ...
    ai.ts                 setAiApiKey() — stores OpenAI-compatible key in SecretStorage (azureBoards.aiApiKey) for the Polish fallback
    polish.ts             isAiAvailable()/polishDraft(): vscode.lm first (no vendor filter), else OpenAI-compatible fallback for editors with no lm provider (Cursor)
...
- The **AI Polish key** is a *separate* secret keyed `azureBoards.aiApiKey` (not the Azure DevOps PAT) — set via `setAiApiKey` (`azureBoards.setAiApiKey`). It's only used for the OpenAI-compatible Polish fallback and only sent to `azureBoards.ai.baseUrl`. The non-secret half (`azureBoards.ai.baseUrl`, `azureBoards.ai.model`) lives in settings, mirroring the org-URL/PAT split.
